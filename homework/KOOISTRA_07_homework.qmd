...
# YOUR CODE HERE
new_formula = bf(log_rt ~ log_frequency +
                     (1 + log_frequency | PartID) +
                     (1 | ItemID))


get_prior(new_formula,quantese)
...
mcmc_plot(xmdl_whatever, type = "trace")
...
pred_draws <- newdata %>%
  add_epred_draws(xmdl_whatever, re_formula = NA)


newdata %>%
  add_epred_draws(xmdl_whatever) %>%
  ggplot(aes(x = log_frequency, y = .epred)) +
  stat_lineribbon(.width = 1) +
  geom_point(data = quantese, aes(y = log_rt), alpha = 0.5) +
  theme_minimal()



#  ATTEMPTED TO USE GUIDANCE FROM 
 # https://mjskay.github.io/tidybayes/reference/add_predicted_draws.html
# COULD NOT FIGURE THIS OUT :( after a lot of times trying 
# struggling to understand this conceptually in all honesty so i am a bit bummed I couldn't figure this out from my research on thisssssss UGH
