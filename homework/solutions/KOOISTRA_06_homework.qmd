...
sim$Sem <- as.factor(sim$Sem)

# PLOT !!!
ggplot(sim, aes(x = Distance, fill = Sem)) +
  geom_density(alpha = 0.4) +
  labs(
    title = "Distance Between the Two Cities by Semantic Categories",
    x = "Distance",
    y = "Distribution"
  ) +
  theme_minimal()
...
##### **Right-skewed distributions, the long tails towards the longer distances**

##### **Most of the data points fall between 40 and 100 - ish**

##### **Peak around 70 - 80 for both categories**
...
clean_sim <- na.omit(sim)
mean(clean_sim$Distance)
sd(clean_sim$Distance)

plot_normal(mean = 77.60331, sd = 40.13778)
```
...
#| label: spec_priors
my_priors <- c(
  prior(normal(80, 40), class = "Intercept"),
...
pp_check(xmdl_plus, ndraws = 100)


```

The peaks are all falling still about the same at the y line shows. There are some strange bumps on the right tail that doesn't seem to be coming from the plotted yrep predictions. Maybe there is something else that we are not accounting for at the moment ????
